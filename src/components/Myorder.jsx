...
const Myorder = () => {
  ...
  const getOrderHistoryByDate = async (date) => {
    await axios
      .get(
        `${api_url}/bills/order-history?messengerId=${messengerId}&date_input=${date}`,
      )
      .then((res) => {
        if (res.status === 200) {
  ...
  return (
    <>
      <Card>
        <Card.Body>
          <Card.Title as={"h6"} className="text-center mb-3">
            คำสั่งซื้อของฉัน
          </Card.Title>
          <Tabs
          ...
          >
                    ...
                    <React.Fragment key={index}>
                      <Card className="mb-4">
                        <Card.Body>
                          ...
                          />
                          <h6 style={{ fontSize: "18px" }}>
                            รวมค่าอาหารทั้งหมด {item.amount} บาท
                          </h6>
                          {item.ordertype === "สั่งกลับบ้าน" && (
                            <h6 style={{ fontSize: "18px" }}>
                              ค่าส่ง {item.delivery_fee} บาท
                          ...
                          <h6 style={{ fontSize: "18px" }}>
                            รวมทั้งหมด{" "}
                            {Number(item.amount) + Number(item.delivery_fee)}{" "}
                            บาท
                          </h6>
                          <h6 style={{ fontSize: "18px" }}>
                            การรับอาหาร -{" "}
            ...
            >
              <Form className="mb-2">
                ...
                <Form.Control
                  type="date"
                  onChange={(e) => getOrderHistoryByDate(e.target.value)}
                />
              </Form>
                  ...
                  <React.Fragment key={index}>
                    <Card className="mb-4">
                      <Card.Body>
                        <h6>
                          {" "}
                          หมายเลขออเดอร์ {item.bill_ID
                            .slice(-5)
                            .toUpperCase()}{" "}
                          <br /> วันที่สั่งออเดอร์ {}
                          {moment(item.Date_times).format("YYYY-MM-DD")}
                          &nbsp; เวลา {moment(item.Date_times).format(
                            "HH:mm",
                          )}{" "}
                          น.
                        </h6>
                        ...
                        <h6 style={{ fontSize: "18px" }}>
                          รวมทั้งหมด{" "}
                          {Number(item.amount) + Number(item.delivery_fee)} บาท
                        </h6>
                        <h6 style={{ fontSize: "18px" }}>
                          การรับอาหาร -{" "}
                          {item.ordertype === "สั่งกลับบ้าน"
