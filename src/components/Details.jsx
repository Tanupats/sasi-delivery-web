...
const Details = (props) => {
  ...
  const getMenuType = async () => {
    await http
      .get(`/menutype/${shop?.shop_id}`, { headers: { apikey: token } })
      .then((res) => {
  ...
  const getMenuByTypeId = async (id) => {
    await http.get(`/foodmenu/${id.value}`).then((res) => {
  ...
  const getDetail = async () => {
    await http.get(`/billsdetails/${bill_ID}`).then((res) => {
      setDetail(res.data);
    });
  };
  ...
  const UpdateDetailById = async () => {
    ...
    await http
      .put(`/billsdetails/${id}`, body, { headers: { apikey: token } })
      .then((res) => {
        if (res) {
          getDetail();
          setOnupdate(true);
  ...
  const addNewMenu = async () => {
    ...
    await http
      .post(`/billsdetails`, body, { headers: { apikey: token } })
      .then((res) => {
        if (res.status === 200) {
          handleClose();
        }
            ...
            <React.Fragment key={item.id}>
              <Row>
                <Col md={12}>
                  ...
                  >
                    {" "}
                    X {item.quantity} {item.foodname} {item.price} <br />{" "}
